    ...
    class Feed {
		...
		public static function getAll($type = 'all', $scope = 'public', $limit = '99', $node = null) {
            ...
            try {
                ...
                $sqlType = '';
                if($type == 'finpop'){
                	$sqlType = " AND feed.type = 'projects' OR feed.type = 'community' ";
                }
                else
                {
	                if ($type != 'all') {
        ...
        public static function subItemTimeLine ($item) {
        	...
        	if (!empty($item->url)) {
        		...
        		if (!empty($item->image)) {
        			...
        			if (substr($item->url, 0, 5) == '/user') {
        				$content .= '<div class="cd-timeline-img cd-location">
        								<a href="'.$item->url.'" class="avatar"><img src="'.SRC_URL.'/image/'.$item->image.'/70/70/1" /></a>
									</div> <!-- cd-timeline-img -->';
        			} else {
        				$content .= '<div class="cd-timeline-img cd-location">
       										<a href="'.$item->url.'" class="image"><img src="'.SRC_URL.'/image/'.$item->image.'/70/70/1" /></a>
									 </div> <!-- cd-timeline-img -->';
        			}
        		} else {
        			// solo titulo con enlace
        			$content .= '<div class="cd-timeline-img cd-location">
									<img src="img/cd-icon-location.svg" alt="Feed">
								</div> <!-- cd-timeline-img -->';
        		}
        	} else {
        		// solo el timeago
        	...
        	// y lo que venga en el html
        	$content.='<div class="cd-timeline-content">
        					<span class="cd-date"><i class="fa fa-clock-o margin-right"></i>'.$pub_timeago.'</span>
        					<a href="'.$item->url.'" class="image-coverture"><img src="'.SRC_URL.'/image/'.$item->image.'/800/600/1" /></a>
							<div class="content">
	        					<h2>'.$item->title.'</h2>
								<p>'.$item->html.'.</p>
								<!--<a href="'.$item->url.'" class="cd-read-more">Savoir Plus</a>-->
							</div>
					  </div> <!-- cd-timeline-content -->';
