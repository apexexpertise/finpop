...
<div class="widget project-message">
    ...
    <div>
        ...
        <form method="post" action="/message/<?php echo $project->id; ?>">
            ...
            <button class="btn btn-default" type="submit"><i class="fa fa-paper-plane margin-right"></i><?php echo Text::get('project-messages-send_message-button'); ?></button>
        </form>
    </div>
</div>
<?php endif; ?>
		...
		<?php foreach ($project->messages as $message) : ?>
                <div class="message<?php if ($message->user->id == $project->owner) echo ' owner'; ?>">
                   ...
                   <a href="/user/profile/<?php echo htmlspecialchars($message->user->id)?>" target="_blank">
                    <img src="<?php echo $message->user->avatar->getLink(50,50, true); ?>" alt="" class="img-circle" />
                   </a>
                  </span>
                   <h<?php echo $level ?> class="user">
				   <i class="fa fa-user margin-right"></i><a href="/user/profile/<?php echo htmlspecialchars($message->user->id)?>" target="_blank">
                   ...
                   <div class="date"> <span><i class="fa fa-clock-o margin-right"></i>Il ya <?php echo $message->timeago ?></span></div>
                   <blockquote><?php echo $message->message; ?></blockquote>
                   <div class="actions">
                        <?php if (!empty($_SESSION['user'])) : ?>
                        <a class="" href="#" onclick="answer('<?php echo $message->id; ?>')"><i class="fa fa-reply margin-right"></i><?php echo Text::get('project-messages-answer_it'); ?></a>
                        <?php endif; ?>
                    ...
                    foreach ($message->responses as $child) : ?>
                       <div class="child<?php if ($child->user->id == $project->owner) echo ' owner'; ?>">
                           ...
                           <a href="/user/profile/<?php echo htmlspecialchars($child->user->id) ?>" target="_blank">
	                           <img src="<?php echo $child->user->avatar->getLink(40, 40, true); ?>" class="img-circle" />
                           </a>
                           </span>
                           <a name="message<?php echo $child->id; ?>" />
                           <h<?php echo $level ?> class="user">
						   <i class="fa fa-user margin-right"></i><a href="/user/profile/<?php echo htmlspecialchars($child->user->id) ?>" target="_blank">
						   <?php echo $child->user->name; ?>
                           </a>
                           </h<?php echo $level ?>>
                           <div class="date"><span><i class="fa fa-clock-o margin-right"></i>il ya <?php echo $child->timeago; ?></span></div>
                           <blockquote><?php echo $child->message; ?></blockquote>
                           <?php // si puede borrar este mensaje
                           if (\Goteo\Core\ACL::check("/message/delete/{$child->id}/{$project->id}")) : ?>
                           <div class="actions">
                                <a href="/message/delete/<?php echo $child->id; ?>/<?php echo $project->id; ?>"><i class="fa fa-times margin-right"></i><?php echo Text::get('regular-delete'); ?></a>
                           </div>
                           <?php endif; ?>
                       </div>
                <?php endforeach;
